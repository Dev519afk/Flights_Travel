...
const CRUISES = [
  ...
  { title: "Norwegian Fjords", duration: "7 nights", price: "£799", rating: 4.7, img: "https://images.unsplash.com/photo-1513519245088-0e12902e35ca?w=500&q=80" },
];

const FLIGHTS = [
  { id: 1, airline: "British Airways", code: "BA", from: "LHR", to: "DXB", dep: "06:30", arr: "16:45", duration: "7h 15m", stops: "Non-stop", price: "£549", class: "Economy" },
  { id: 2, airline: "Emirates", code: "EK", from: "LHR", to: "DXB", dep: "09:15", arr: "20:00", duration: "7h 45m", stops: "Non-stop", price: "£689", class: "Business" },
  { id: 3, airline: "Virgin Atlantic", code: "VS", from: "LHR", to: "DXB", dep: "13:45", arr: "00:30+1", duration: "8h 45m", stops: "1 Stop", price: "£429", class: "Economy" },
  { id: 4, airline: "Qatar Airways", code: "QR", from: "LHR", to: "DXB", dep: "16:20", arr: "03:05+1", duration: "9h 45m", stops: "1 Stop", price: "£399", class: "Economy" },
  { id: 5, airline: "Etihad Airways", code: "EY", from: "LHR", to: "DXB", dep: "22:10", arr: "08:55+1", duration: "7h 45m", stops: "Non-stop", price: "£619", class: "Economy" },
];
...
function useRouter() {
  ...
  const navigate = (p, data) => { setPage(p); if (data) setSelectedFlight(data); window.scrollTo(0, 0); };
  return { page, navigate, selectedFlight };
}
  ...
  return (
    <div style={{ background: "#061929", padding: "8px 24px", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 8 }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
        ...
        <span style={{ color: "#94a3b8", fontSize: 12 }}>0800 123 4567</span>
      </div>
    </div>
  );
}
    ...
    <div style={{ background: "rgba(255,255,255,0.08)", backdropFilter: "blur(20px)", border: "1px solid rgba(255,255,255,0.15)", borderRadius: 20, padding: "28px 32px", maxWidth: 860, width: "100%", boxShadow: "0 25px 60px rgba(0,0,0,0.4)" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: 12, marginBottom: 16 }}>
        {[
          ...
          { label: "Dates", icon: "calendar", val: "15 Jun – 22 Jun", setVal: () => {}, placeholder: "Select dates" },
      ...
      <button onClick={() => navigate("flights")} style={{ width: "100%", background: `linear-gradient(135deg, ${C.orange}, #e55a00)`, border: "none", color: "#fff", padding: "16px 32px", borderRadius: 12, cursor: "pointer", fontWeight: 700, fontSize: 16, letterSpacing: "0.3px", boxShadow: "0 8px 24px rgba(255,122,26,0.5)", transition: "transform 0.2s" }}
  ...
  return (
    <div style={{ background: C.light, minHeight: "100vh" }}>
      ...
      <div style={{ background: `linear-gradient(135deg, ${C.navy}, #0f3460)`, padding: "32px 24px" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          <p style={{ color: "rgba(255,255,255,0.6)", fontSize: 13, marginBottom: 6 }}>Search results</p>
          <h1 style={{ color: "#fff", fontSize: 28, fontWeight: 800, fontFamily: "'Poppins', sans-serif" }}>
            London → Dubai
          </h1>
          <p style={{ color: "rgba(255,255,255,0.7)", fontSize: 14, marginTop: 8 }}>
            15 Jun · 1 Adult · {filtered.length} flights found
          </p>
        </div>
      </div>

      <div style={{ maxWidth: 1200, margin: "0 auto", padding: "32px 24px", display: "grid", gridTemplateColumns: "280px 1fr", gap: 24, alignItems: "start" }}>
        {/* Sidebar */}
        <div style={{ background: "#fff", borderRadius: 16, padding: "24px", boxShadow: "0 2px 12px rgba(0,0,0,0.06)", border: "1px solid #e2e8f0", position: "sticky", top: 90 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 24 }}>
            <Icon name="filter" size={18} color={C.orange} />
            <span style={{ fontWeight: 700, fontSize: 16, color: C.text }}>Filters</span>
          </div>

          <div style={{ marginBottom: 28 }}>
            <div style={{ fontWeight: 600, fontSize: 14, color: C.text, marginBottom: 12 }}>Max Price</div>
            <input type="range" min={300} max={800} value={priceMax} onChange={e => setPriceMax(Number(e.target.value))} style={{ width: "100%", accentColor: C.orange }} />
            <div style={{ display: "flex", justifyContent: "space-between", marginTop: 6 }}>
              <span style={{ color: C.muted, fontSize: 13 }}>£300</span>
              <span style={{ fontWeight: 700, color: C.orange, fontSize: 14 }}>£{priceMax}</span>
            </div>
          </div>

          <div style={{ marginBottom: 28 }}>
            <div style={{ fontWeight: 600, fontSize: 14, color: C.text, marginBottom: 12 }}>Stops</div>
            {[["all","All Flights"],["nonstop","Non-stop only"],["1stop","1 Stop"]].map(([k,l]) => (
          ...
          <div>
            <div style={{ fontWeight: 600, fontSize: 14, color: C.text, marginBottom: 12 }}>Airlines</div>
            {airlines.map(a => (
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          {filtered.length === 0 && (
            <div style={{ textAlign: "center", padding: "60px 24px", background: "#fff", borderRadius: 16 }}>
              <p style={{ color: C.muted, fontSize: 16 }}>No flights match your filters. Try adjusting them.</p>
            </div>
          )}
          {filtered.map(f => (
            <div key={f.id} style={{ background: "#fff", borderRadius: 16, padding: "24px 28px", boxShadow: "0 2px 12px rgba(0,0,0,0.06)", border: "1px solid #e2e8f0", display: "grid", gridTemplateColumns: "1fr auto auto", alignItems: "center", gap: 24 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 24 }}>
                <div style={{ width: 48, height: 48, borderRadius: 12, background: "linear-gradient(135deg, #0a2540, #0f3460)", display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontWeight: 800, fontSize: 13, flexShrink: 0 }}>{f.code}</div>
                <div>
                  <div style={{ fontWeight: 700, fontSize: 14, color: C.text, marginBottom: 2 }}>{f.airline}</div>
                  <div style={{ color: C.muted, fontSize: 12 }}>{f.class}</div>
                </div>
                <div style={{ display: "flex", alignItems: "center", gap: 20 }}>
                  <div style={{ textAlign: "center" }}>
                    <div style={{ fontWeight: 800, fontSize: 22, color: C.text }}>{f.dep}</div>
                    <div style={{ color: C.muted, fontSize: 12, fontWeight: 600 }}>{f.from}</div>
                  </div>
                  <div style={{ textAlign: "center" }}>
                    <div style={{ color: C.muted, fontSize: 11, marginBottom: 4 }}>{f.duration}</div>
                    <div style={{ width: 80, height: 1, background: "#e2e8f0", position: "relative" }}>
                      <div style={{ position: "absolute", top: -4, left: "50%", transform: "translateX(-50%)", color: C.orange }}>
                        <Icon name="plane" size={10} color={C.orange} />
                      </div>
                    </div>
                    <div style={{ color: f.stops === "Non-stop" ? "#10b981" : "#f59e0b", fontSize: 11, fontWeight: 600, marginTop: 4 }}>{f.stops}</div>
                  </div>
                  <div style={{ textAlign: "center" }}>
                    <div style={{ fontWeight: 800, fontSize: 22, color: C.text }}>{f.arr}</div>
              ...
              <div style={{ textAlign: "right" }}>
                ...
                <div style={{ fontWeight: 900, fontSize: 26, color: C.orange }}>{f.price}</div>
              </div>
              <button onClick={() => navigate("booking", f)} style={{ background: `linear-gradient(135deg, ${C.orange}, #e55a00)`, border: "none", color: "#fff", padding: "12px 24px", borderRadius: 10, cursor: "pointer", fontWeight: 700, fontSize: 14, boxShadow: "0 4px 16px rgba(255,122,26,0.4)", transition: "transform 0.2s" }}
                onMouseEnter={e => e.target.style.transform = "translateY(-2px)"}
                onMouseLeave={e => e.target.style.transform = "translateY(0)"}>
                Book Now
              </button>
...
export default function App() {
  const { page, navigate, selectedFlight } = useRouter();
  return (
    <div style={{ fontFamily: "'Inter', 'Poppins', system-ui, sans-serif", color: C.text, minHeight: "100vh" }}>
      ...
      {page === "flights" && <FlightsPage navigate={navigate} />}
